    ...
    Color3f Li(const Scene *scene, Sampler *sampler, const Ray3f &ray) const {
       ...
       // PointLight light()
        for(auto Light: scene->getLights())
        {
             ...
             Point2f sample2D(rand()/RAND_MAX, rand()/RAND_MAX);
            ...
            Ray3f l(its.p, Global_wi, Epsilon, emit.dist);
            ...
            if(!scene->rayIntersect(l))
            {
                ...
                float cos = sh_normal.dot(Global_wi);  
                cos = (cos > 0)? cos:0;
                ...
                Vector3f Local_wi = its.toLocal(Global_wi);
