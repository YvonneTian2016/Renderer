...
public:
    ...
    float PDF_get() const { return uni_pdf; }
    
    void PDF_compute() {
        
       // cout<<"pdf_compute"<<endl;
        int Tri_C = m_F.cols();
        DiscretePDF Tri_pdf = DiscretePDF(Tri_C);
        for (int i = 0; i < Tri_C; i++){
            Tri_pdf.append(surfaceArea(i));
        }
        uni_pdf = 1/Tri_pdf.normalize();
        m_pdf = Tri_pdf;
        setPDF = true;
    }
...
protected:
    DiscretePDF m_pdf;                   //The pdf of this mesh
    bool setPDF = false;                 // if the pdf of this mesh is set or not
    float uni_pdf;                       //  normalized unit of pdf 1->uni_pdf
